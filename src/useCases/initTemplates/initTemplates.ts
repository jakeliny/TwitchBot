 ...
 */
const getTemplateChannel: any = async (channel = "") => {
    ...
    const [channelCommands, commonCommands] = await Promise.all([
        ...
        promises.readFile(path.resolve(`commands.${owner}.json`)).catch(() => null),
        promises.readFile(path.resolve(`commands.json`)).catch(() => null),
    ])
...
export default getTemplateChannel